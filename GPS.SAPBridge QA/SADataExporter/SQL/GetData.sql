BEGIN
    ...
    BEGIN TRY
        ...
        MERGE INTO TB_R_GR_IR gr USING (
            ...
            WHERE gri.SAP_MAT_DOC_NO IS NULL AND STATUS_CD IN ('60','68')
            AND poi.ITEM_CLASS = 'S' AND COMPONENT_PRICE_CD = 'PB00'
            AND gri.PROCESS_ID IS NULL) tmp
        ON gr.PO_NO = tmp.PO_NO AND gr.PO_ITEM = tmp.PO_ITEM AND gr.PO_SUBITEM_NO = tmp.PO_SUBITEM_NO AND gr.MAT_DOC_NO = tmp.MAT_DOC_NO
        WHEN MATCHED THEN
