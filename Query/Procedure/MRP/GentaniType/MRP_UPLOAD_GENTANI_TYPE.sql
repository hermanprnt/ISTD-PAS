USE [NCP_QA]
GO
/****** Object:  StoredProcedure [dbo].[MRP_UPLOAD_GENTANI_TYPE]    Script Date: 10/6/2015 9:54:19 AM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[MRP_UPLOAD_GENTANI_TYPE]
    @USER_ID VARCHAR(50),
	@PROCESS_ID numeric	   		
AS
SET NOCOUNT ON
...
BEGIN TRY
   ...
   EXEC SP_WRITE_LOG_D @USER_ID,@PROCESS_ID, 'Start Upload', 'GPS0004INF', 'Upload Gentani Type', @N_ERR
		
--CHECK VALIDASI						   
-----------------
	SELECT @maxid = COUNT(1) FROM TB_T_GENTANI_TYPE
	SET @Flag = 1
	...
	WHILE (@Flag <= @maxid)
		BEGIN 
			...
			FROM TB_T_GENTANI_TYPE 
			WHERE NO=@Flag	
			
			IF (@PROC_USAGE_CD = '' AND @GENTANI_HEADER_TYPE = '' AND @DESCRIPTION = '')				
				BEGIN
					SET @L_ERR = 1	
			...
			IF LEN(@DESCRIPTION) > 50
				BEGIN
					SET @L_ERR = 1	
					SET @MSG_TEXT = 'False|DESCRIPTION not more 50 character'		
					SET @MSG_TEXT = @MSG_TEXT + ' At row' + CAST(@LINE AS VARCHAR)
					EXEC SP_WRITE_LOG_D @USER_ID, @PROCESS_ID, 'Check Data Length', 'GPS0004ERR', @MSG_TEXT, @N_ERR										
				END						
				
			--CHECK DUPLICATE DATA 
			SET @ERR_MSG_DUPLACATE_COUNT = (SELECT COUNT(1) FROM TB_T_GENTANI_TYPE 
			...
			IF (@ERR_MSG_DUPLACATE_COUNT > 1)
				...
				END	
            SET @ERR_MSG_DUPLACATE_COUNT = (SELECT COUNT(1) FROM TB_M_GENTANI_TYPE 
										     WHERE PROC_USAGE_CD = @PROC_USAGE_CD AND
												   GENTANI_HEADER_TYPE = @GENTANI_HEADER_TYPE 
										    ) 
...
IF (@L_ERR = 0)
	BEGIN
		INSERT INTO [dbo].[TB_M_GENTANI_TYPE]
							   ([PROC_USAGE_CD]
							   ,[GENTANI_HEADER_TYPE]
							   ,[DESCRIPTION]					   
							   ,CREATED_BY
		...
		SELECT  PROC_USAGE_CD,
				...
				@USER_ID,
				GETDATE()
        FROM [dbo].[TB_T_GENTANI_TYPE]
		
		EXEC SP_WRITE_LOG_D @USER_ID,@PROCESS_ID, 'Finish Uploading', 'GPS0004SUC', 'Finish Uploading Gentani Type', @N_ERR
