...
-- Description : Get List PR Approval List for Accounting Roles  Copy From sp_PRApproval_Get
-- ===================================================================  
IF EXISTS (SELECT * FROM   sys.objects WHERE  object_id = OBJECT_ID(N'[dbo].[sp_PRApproval_CountListForAccounting]') AND type IN ( N'P', N'PC' ))
BEGIN
  DROP PROCEDURE [dbo].[sp_PRApproval_CountListForAccounting]
END
GO

CREATE PROCEDURE [dbo].[sp_PRApproval_CountListForAccounting]
    @docNo VARCHAR(11),
	@docType VARCHAR(5),
	@docDesc VARCHAR(100),
	@dateFrom DATETIME,
...
BEGIN
    ...
    FROM
    (
        SELECT DISTINCT TOP (@MAX_RECORD) PRH.PR_NO AS 'DOC_NO', PRI.PR_ITEM_NO AS 'DOC_ITEM_NO'
            FROM TB_R_PR_H PRH
