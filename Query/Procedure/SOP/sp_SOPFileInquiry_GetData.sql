...
GO
CREATE PROCEDURE [dbo].[sp_SOPFileInquiry_GetData]
	@docId VARCHAR(100)
AS
BEGIN
	SELECT 
		[DOCUMENT_ID], 
		(SELECT TOP 1 SYSTEM_VALUE FROM TB_M_SYSTEM WHERE FUNCTION_ID ='00000' AND SYSTEM_CD ='UPLOAD_FILE_PATH')+
		(SELECT TOP 1 SYSTEM_VALUE FROM TB_M_SYSTEM WHERE FUNCTION_ID ='00000' AND SYSTEM_CD ='SOP_PATH') + 
		[FILE_NAME] AS [FILE_NAME], 
		[FILE_NAME_ORI], 
		[FILE_DESC], 
		[FILE_EXTENSION],
		CASE WHEN LOWER([FILE_EXTENSION]) = '.pdf' THEN 'pdf.png'
				WHEN LOWER([FILE_EXTENSION]) = '.ppt' THEN 'ppt.png'
		...
		CREATED_BY,
		CREATED_DT
	FROM TB_M_SOP_DOCUMENT WHERE DOCUMENT_ID = @docId
