...
{
    public abstract class BudgetWindowsService : ServiceBase
    {
        ...
        protected BudgetWindowsService()
        {
        ...
        private void CheckApproachingSchedule()
        {
            try
            {
                ...
                CurrentTask.ScheduleList = repo.GetScheduleList(CurrentTask.SystemType, CurrentTask.SystemCd);
                logtemp.Add("Get schedule list: end");

                var approaching = GetApproachingSchedule(CurrentTask, isIncludeOneMinuteBehind: true);
                IList<RegisteredSchedule> registeredSchList = repo.GetRegisteredSchedule(approaching.SystemType);
                ...
                {
                    ProcessId = ProcessId,
                    SystemType = approaching.SystemType,
                ...
                if (isAnyQueue)
                {
                    ...
                    queued.SystemType = approaching.SystemType;
        ...
        private Schedule GetApproachingSchedule(ITask currentTask, Boolean isIncludeOneMinuteBehind)
        {
            ...
            IList<Schedule> timeSpanList = GenerateTimeSpanList(currentTask.SystemType, scheduleList, isForToday: true);
            ...
            if (!inTimeFrameScheduleList.Any())
                timeSpanList = GenerateTimeSpanList(currentTask.SystemType, scheduleList, isForToday: false);
        ...
        private IList<Schedule> GenerateTimeSpanList(String SystemType, IList<NameValueItem> scheduleList, Boolean isForToday)
        {
            return scheduleList
                .Select(tl =>
                    new Schedule
        ...
        public void StartScheduler(string log)
        {
            ...
            ProcessId = repo.GetNewProcessId();
