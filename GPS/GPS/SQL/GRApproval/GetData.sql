...
(
select A.MAT_DOC_NO,DOCUMENT_DT,HEADER_TEXT,A.VENDOR_CD,A.PO_NO,ITEM_CURRENCY,A.STATUS_CD,M.VENDOR_NAME
,SUM(A.GR_IR_AMOUNT) AMOUNT,MAX(A.CHANGED_BY) CHANGED_BY, MAX(A.CHANGED_DT) CHANGED_DT
,MAX(A.CREATED_BY) CREATED_BY, MAX(A.CREATED_DT) CREATED_DT,MAX(E.ATTACHMENT_DOC_PATH) ATTACHMENT_DOC_PATH, MAX(E.ATTACHMENT_DOC_FILE_NM) ATTACHMENT_DOC_FILE_NM
,MAX(E.OTHERS_ATTACHMENT_DOC_PATH) OTHERS_ATTACHMENT_DOC_PATH, MAX(E.OTHER_ATTACHMENT_DOC_FILE_NM) OTHER_ATTACHMENT_DOC_FILE_NM, MAX(ST.STATUS_DESC) STATUS
from TB_R_GR_IR A
...
left join TB_M_STATUS ST ON A.STATUS_CD=ST.STATUS_CD
cross apply #s S 
...
GROUP BY A.MAT_DOC_NO,DOCUMENT_DT,HEADER_TEXT,A.VENDOR_CD,A.PO_NO,ITEM_CURRENCY,A.STATUS_CD,M.VENDOR_NAME
) t1
) tb where seq_no between @RowStart and @RowEnd order by seq_no asc
