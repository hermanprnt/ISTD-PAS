...
SET @@B=(SELECT COUNT(1) FROM TB_R_PO_H WHERE PO_NO=@@PO OR REF_NO=@@REFF);
	...
	BEGIN
			...
			BEGIN
				...
				ELSE
				BEGIN
					IF (SELECT DISTINCT 1 FROM TB_R_PO_ITEM WHERE PO_NO=@@PO AND ITEM_CLASS='M')=1
					BEGIN
						SELECT @@PO_NO = PO_NO, @@WBS = WBS_NO FROM TB_R_PO_ITEM WHERE PO_NO=@@PO AND ITEM_CLASS='M' -- PO_NO AND WBS
						IF EXISTS (SELECT PO_NO FROM TB_M_WBS_CARRYOVER WHERE PO_NO = @@PO_NO)
						BEGIN
							IF EXISTS (SELECT WBS_NO FROM TB_M_WBS_CARRYOVER WHERE PO_NO = @@PO_NO AND WBS_NO = @@WBS)
							BEGIN
					...
					ELSE
					BEGIN
						SELECT '3' -- PO IS NOT FOR GR
					END
				END
			END
			ELSE IF @@PO=''
			BEGIN	
				SET @@A=(SELECT COUNT(*) FROM TB_R_PO_H WHERE ref_no=@@REFF  AND PO_STATUS='44');
				--SET @@PO=@@POREFF;
			END
			IF @@A>1
			BEGIN 
