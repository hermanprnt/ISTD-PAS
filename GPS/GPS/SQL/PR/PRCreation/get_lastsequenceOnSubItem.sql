...
BEGIN
	...
	SELECT TOP 1 REPLICATE('0', 5-LEN(CAST((ISNULL(MAX(CAST(SUBITEM_NO AS INT)),0) + @@MULTIPLY) AS VARCHAR(5)))) + CAST((ISNULL(MAX(CAST(SUBITEM_NO AS INT)),0) + @@MULTIPLY) AS VARCHAR(5)) AS SEQNO 
	FROM TB_T_PR_SUBITEM 
