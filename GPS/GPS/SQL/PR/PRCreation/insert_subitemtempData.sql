DECLARE @@SEQ_NO_EXIST INT,
		...
		@@MESSAGE VARCHAR(MAX) = '',
		--FID.Ridwan: 20211123
		@@COST_GL_EXIST_SUBITEM VARCHAR(100)
...
BEGIN TRY
	...
	--FID.Ridwan: 20211123
	SELECT TOP 1 @@COST_GL_EXIST_SUBITEM = ISNULL(COST_CENTER_CD, '') + '' + ISNULL(GL_ACCOUNT, '') FROM TB_T_PR_SUBITEM WHERE PROCESS_ID = @PROCESS_ID AND SUBITEM_NO != @SUBITEM_NO AND ITEM_NO = @ITEM_NO AND DELETE_FLAG = 'N'
	...
	BEGIN
		...
		END
		--FID.Ridwan: 20211123
		IF((ISNULL(@COST_CENTER_CD,'') + '' + ISNULL(@GL_ACCOUNT, '')) != @@COST_GL_EXIST_SUBITEM )
		BEGIN
			SET @@MESSAGE = 'Sub item for Item Class Service must Choose the same Combination Cost Center and GL Account'
			RAISERROR(@@MESSAGE, 16, 1)
		END
	END
	...
	INSERT INTO [dbo].[TB_T_PR_SUBITEM]
		...
		VALUES (
			...
			0, --ORI_LOCAL_AMOUNT
			case when @@RATE is null then 0 else (@SUBITEM_QTY * @PRICE_PER_UOM * @@CALC_VALUE) * @@RATE end, --NEW_LOCAL_AMOUNT
			@USERID,
			GETDATE(),
			NULL,
			NULL,
	...
	BEGIN
		...
		UPDATE TB_T_PR_SUBITEM SET 
				...
				NEW_LOCAL_AMOUNT = (@SUBITEM_QTY * @PRICE_PER_UOM ) * @@RATE,
				CHANGED_BY = @USERID, 
				CHANGED_DT = GETDATE(),
				UPDATE_FLAG = 'Y'
	...
	BEGIN
		...
		UPDATE TB_T_PR_ITEM
			SET NEW_AMOUNT = @@ITEM_AMOUNT, NEW_LOCAL_AMOUNT = @@ITEM_AMOUNT * EXCHANGE_RATE
