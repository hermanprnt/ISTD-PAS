BEGIN
    DECLARE
        @@message VARCHAR(MAX),
        @@actionName VARCHAR(50) = 'SADataExporter_GetData.sql',
        @@tmpLog LOG_TEMP,
    ...
    BEGIN TRY
        ...
        MERGE INTO TB_R_GR_IR gr USING (
            SELECT gri.PO_NO,gri.PO_ITEM, gri.MAT_DOC_NO FROM TB_R_GR_IR gri 
            ...
            AND poi.ITEM_CLASS = 'S' AND COMPONENT_PRICE_CD = 'PB00'
            AND gri.PROCESS_ID IS NULL
			GROUP BY gri.PO_NO,gri.PO_ITEM, gri.MAT_DOC_NO) tmp
	...
	SELECT
		...
		ISNULL(gr.PO_SUBITEM_NO, '') + '${tab}' +       
        ISNULL(REPLACE(ISNULL(CAST(CAST(gr.MOVEMENT_QTY AS DECIMAL(9, 3)) AS VARCHAR), ''), '.', ','), '')         
         /* SA Recipient */ DataRow
    FROM TB_R_GR_IR gr
    ...
    --    ISNULL(gr.MAT_DOC_NO, '')  + '${tab}' + /* MAT_DOC_NO = REF_DOC_NO = GR Number */
    --    ISNULL(gr.HEADER_TEXT, '') + '${tab}' +
    --    ISNULL(REPLACE(ISNULL(CAST(CAST(gr.MOVEMENT_QTY AS DECIMAL(9, 3)) AS VARCHAR), ''), '.', ','), '') + '${tab}' +
    --    ISNULL(gr.UNIT_OF_MEASURE_CD, '') + '${tab}' +
    --    ISNULL(gr.PO_NO, '') + '${tab}' +
    --    ISNULL(gr.PO_ITEM, '') + '${tab}' +
    --    ISNULL(gr.CREATED_BY, '') /* SA Recipient */ DataRow
    --FROM TB_R_GR_IR gr
    --WHERE gr.PROCESS_ID = @ProcessId AND gr.COMPONENT_PRICE_CD = 'PB00'
END
